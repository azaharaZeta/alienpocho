...
test("GUARDARRAÍL: render.js dibuja TODO (objetos Y cáscara) por el motor genérico, no por nombre", () => {
  ...
  // La cáscara (paredes/puertas) va por roomShell + AP.drawAsset, IGUAL que los objetos. Solo se permite por
  // nombre el ÚNICO pre-pase de fondo z=0 (AP.floor) + AP.drawSprite (icono HUD). (AP.doorHole ya no existe;
  // se conserva en la lista como guardarraíl por si se reintrodujera.)
